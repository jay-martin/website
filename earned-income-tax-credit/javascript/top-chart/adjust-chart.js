    ...
    if(filingStatus === 'married'){
        ...
        setTimeout(function(){
            if(filingStatus === 'single' || filingStatus === 'hoh'){
                single_eitc_builder_2023(top_chart_chart, 'x', 'eitc', numChildren);
            }
            else if(filingStatus === 'married'){
                married_eitc_builder_2023(top_chart_chart, 'x', 'eitc', numChildren);
            }
            modify_income();
        }, 400);
    }
...
function top_chart_adjust_y_axis(){
    ...
    // adjust y-axis ticks
    if(numChildren === 'none'){
        top_chart_chart.internal.config.axis_y_tick_values = [0, 200, 400, 600, 800, 1000];
    }
    else{
        top_chart_chart.internal.config.axis_y_tick_values = [0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000];
    }

    // adjust y-axis max
    if(numChildren === 'none'){
        top_chart_chart.axis.max({y: 1000});
    }
    else if(numChildren === 'one'){
        top_chart_chart.axis.max({y: 5500});
    }
    else if(numChildren === 'two'){
        top_chart_chart.axis.max({y: 8500});
    }
    else{
        top_chart_chart.axis.max({y: 10000});
    }
}
...
function top_chart_adjust_x_axis(){
    ...
    if(numChildren === 'none'){
        ...
        top_chart_chart.internal.config.axis_x_tick_values = [0, 10000, 20000, 30000, 40000, 50000, 60000, 65000];
    }

    // adjust x-axis max
    if(numChildren === 'none' && filingStatus === 'single'){
        top_chart_chart.axis.max({x: 20000});
        ...
        top_chart_chart.axis.max({x: 55000});
        ...
        top_chart_chart.axis.max({x: 65000});
        user_income.max = 65000;
    }
}

function top_chart_description_generator(){
    let top_filing_status = capitalize_filing_status(filing_status.value);
    let num_children = capitalize_num_children(top_num_children.value);

    document.getElementById('top_chart_title_description').innerHTML = top_filing_status + ', ' + num_children;
}
