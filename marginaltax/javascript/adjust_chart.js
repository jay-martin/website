...
function load_emtr_data(){
	...
	for (var i = 0; i < xVals.length; i++) {
		taxRates = tax_and_transfer_at_income_marginal(xVals[i], numChildren);
...
function adjust_axes(){
	if(chart_type.value === 'EMTR'){
        ...
        chart.axis.labels({y: 'Income after Taxes and Transfers'});
        if(zoom_switch.checked === true){
            ...
            chart.internal.config.axis_x_tick_values = [0, 100000, 200000, 300000, 400000, 500000, 600000];
            chart.axis.max({x: 600000});
            chart.internal.config.axis_y_tick_values = [-600000, -500000, -400000, -300000, -200000, -100000, 0, 100000, 200000];
...
function add_tangent_line(income, numChildren){
    ...
    /* return [['x_tangent', 0, 600000], ['tangent_line', y0, yEnd]]; */

    chart.load({columns: [['x_tangent', 0, 600000], ['tangent_line', y0, yEnd]] });
}
